...
class Aurel_Table_AnnuaireCategorie extends Aurel_Table_Abstract 
{
	...
	public function getAll($admin = false){
		...
		->from(['c'=>'annuaire_categorie'])
		->joinLeft(['sc'=>'annuaire_sous_categorie'], 'c.id_annuaire_categorie = sc.id_annuaire_categorie',['sous_categorie_basename'=>new Zend_Db_Expr('GROUP_CONCAT(DISTINCT sc.basename ORDER BY `sc`.`order` ASC,`sc`.`id_annuaire_sous_categorie` DESC)'), 'sous_categorie_name'=>new Zend_Db_Expr('GROUP_CONCAT(DISTINCT sc.name ORDER BY `sc`.`order` ASC,`sc`.`id_annuaire_sous_categorie` DESC)'), 'sous_categorie_id'=>new Zend_Db_Expr('GROUP_CONCAT(DISTINCT sc.id_annuaire_sous_categorie ORDER BY `sc`.`order` ASC,`sc`.`id_annuaire_sous_categorie` DESC)'), 'id_creation'=>new Zend_Db_Expr('GROUP_CONCAT(DISTINCT concat(sc.id_annuaire_sous_categorie,\':\',sc.id_user_creation) ORDER BY `sc`.`order` ASC,`sc`.`id_annuaire_sous_categorie` DESC)')])
		->order('c.order ASC')
		->order('sc.order ASC')
