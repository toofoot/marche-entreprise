...
class Aurel_Table_SondageReponseQuestion extends Aurel_Table_Abstract
{
    ...
    public function getSynthese($id_sondage){
        $select = $this->select()
            ->from(['srq'=>'sondage_reponse_question'],['sessid', 'date', 'id_sondage_question', 'id_user_reponse', 'reponse'=>new Zend_Db_Expr('GROUP_CONCAT(reponse)')])
            ->joinInner(['sq'=>'sondage_question'],'sq.id_sondage_question = srq.id_sondage_question',[])
            ->joinInner(['s'=>'sondage'],'s.id_sondage = sq.id_sondage',[])
    ...
    public function getBySondageAndUser($id_sondage, $id_user){
        $select = $this->select()
            ->from(['srq'=>'sondage_reponse_question'])
            ->joinInner(['sq'=>'sondage_question'],'sq.id_sondage_question = srq.id_sondage_question',[])
            ->joinInner(['s'=>'sondage'],'s.id_sondage = sq.id_sondage',[])
            ->where('s.id_sondage = ?',$id_sondage)
