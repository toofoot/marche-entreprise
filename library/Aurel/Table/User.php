...
class Aurel_Table_User extends Aurel_Table_Abstract {
    ...
    public function getAll($status = null, $search = null, $orderby = 'status', $order = 'ASC', $typeBinaire = null) {
        $select = $this->select();
        if ($typeBinaire !== null) {
            $select->from("user", array('*', 'type_binaire' => new Zend_Db_Expr("REVERSE(RIGHT(concat('0000000000',BIN(`type`)),11))")));
        }
        if ($status !== null)
            $select->where("status = ?", $status);

        if ($search) {
            $select->where("name like ? OR lastname like ? OR firstname like ? OR email like ?", '%' . $search . '%');
        }
